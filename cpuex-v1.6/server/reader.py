...
def sld_reader(sld_file_name: str, endian: str='<'):
    ...
    # 整形されたデータを表示する (オプション)
    lines = []
    for n in sld.sld_data:
        if isinstance(n, float) and n.is_integer():
            lines.append(f"{n:.1f}")
        else:
            lines.append(str(n))

    # 整形されたデータを保存するファイル (任意: デバッグ用)
    formatted_output_file = "formatted_sld_data.txt"
...
endian = '<'  # Little-endian または '>' で Big-endian

# data_reader関数を呼び出してバイナリデータを取得
binary_data = data_reader(file_name, endian)

# バイナリデータをファイルに書き込む
with open('output.bin', 'wb') as f:
    f.write(binary_data)

print("ファイルが変換され、output.binに保存されました。")